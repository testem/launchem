...
function knownBrowsers(config) {
  var userHomeDir = config.homeDir;
  ...
  var browsers = [
    {
      ...
      setup: function(config, done) {
        ...
        if (config.firefox_user_js) {
          return fs.readFile(config.firefox_user_js, function(err, content) {
            if (err) {
              return done(err);
            }
